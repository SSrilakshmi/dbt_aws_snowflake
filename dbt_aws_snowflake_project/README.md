...
- [DBT + AWS + Snowflake Data Engineering Project - Airbnb](#dbt--aws--snowflake-data-engineering-project---airbnb)
  ...
  - [Key Learning Outcomes](#key-learning-outcomes)
    ...
    - [5.Incremental loading](#5incremental-loading)
...
#### Ingestion
![S3 to External Stage](diagrams/architecture_images/s3_staging.png width="60%"/>)
- .csv files are staged for accessing
- secure access using snowflake **integration object** and **aws trust policy**

#### Bronze Layer (Raw Data)
![Data flow from external stage to stage layer](diagrams/architecture_images/staging_bronze.png)
...
dbt_aws_snowflake
  ...
  └── 📁setup
      └── 📁environment
      &#124 ├── setup_env.ps1                       # script to set up environment variables
      └── 📁infrastructure
          ├── 1_airbnb_setup.sql                  # script for db, schema, tables, role, grants, stage, file_format, storage_integration  for multiple environments (dev/test/prod)
          └── 2_airbnb_copy.sql                   # copy to stage and verify counts
...
4. Establish Secure Connecction for snowflake
...
5. Ingest data from AWS - s3 to Snowflake-Stage
     - Run **2_airbnb_copy.sql** available in /setup/environment/infrastruture/ in Snowflake for raw data to be available in External Stage
...
7. Create **Environments** and **Environment variables**
    ...
    - Run script to set environment variables. Note - **dev** following the script path indicates the environment of interest
      ...
      ![Setting environment variables](diagrams/architecture_images/set_environment_variables.png)
...
### 2. Environment-based deployment workflows 
  ...
  - Final output:
      ```sql
      select *
      from DB_AIRBNB_DEV.staging.bookings    
